namespace Utterance
{
    ...
    {
        protected byte[] Bytes => Stream != null ? Stream.ToArray() : new byte[0];

        protected bool IncludePropertyNames { get; set; }
        ...
        {
            WriteBytes(ToBytes(node.NodeType), ToBytes(node.IsLifted), ToBytes(node.IsLiftedToNull),
                ToBytes(node.Method));
        ...
        protected override Expression VisitNew(NewExpression node)
        {
            ...
            if (node.Members.Count > 0) WriteBytes(node.Members.SelectMany(m => ToBytes(m)).ToArray());
        ...
        {
            WriteBytes(ToBytes(node.NodeType), ToBytes(node.Type), ToBytes(node.IsByRef),
                IncludePropertyNames ? ToBytes(node.Name) : NullValue);
        ...
        protected static readonly byte[] NullValue = {0};
        ...
        protected static byte[] ToBytes(Enum value)
        {
            ...
            if (Converters.EnumConverters.TryGetValue(underlyingType, out converter)) return converter(value);
            return new byte[0];
        }
        ...
        protected IByteConverter GetConverter(Type keyType)
        {
            ...
            if (!map.TryGetValue(keyType, out converter)) converter = map[typeof(object)];
        ...
        {
            public static readonly IDictionary<Type, Func<Enum, byte[]>> EnumConverters =
                new Dictionary<Type, Func<Enum, byte[]>>
                {
                    ...
                    {
                        typeof(double),
                        input => BitConverter.GetBytes((double) Convert.ChangeType(input, typeof(double)))
                    },
                    {typeof(short), input => BitConverter.GetBytes((short) Convert.ChangeType(input, typeof(short)))},
                    {typeof(int), input => BitConverter.GetBytes((int) Convert.ChangeType(input, typeof(int)))},
                    {typeof(long), input => BitConverter.GetBytes((long) Convert.ChangeType(input, typeof(long)))},
                    {typeof(float), input => BitConverter.GetBytes((float) Convert.ChangeType(input, typeof(float)))},
                    {
                        typeof(ushort),
                        input => BitConverter.GetBytes((ushort) Convert.ChangeType(input, typeof(ushort)))
                    },
                    {typeof(uint), input => BitConverter.GetBytes((uint) Convert.ChangeType(input, typeof(uint)))},
                    {typeof(ulong), input => BitConverter.GetBytes((ulong) Convert.ChangeType(input, typeof(ulong)))}
                };
            ...
            public static readonly IDictionary<Type, IByteConverter> ByteConverters =
                new Dictionary<Type, IByteConverter>
                {
                    ...
                    {typeof(short), new DelegatingByteConverter(input => BitConverter.GetBytes((short) input))},
                    {typeof(int), new DelegatingByteConverter(input => BitConverter.GetBytes((int) input))},
                    {typeof(long), new DelegatingByteConverter(input => BitConverter.GetBytes((long) input))},
                    {typeof(float), new DelegatingByteConverter(input => BitConverter.GetBytes((float) input))},
                    {typeof(ushort), new DelegatingByteConverter(input => BitConverter.GetBytes((ushort) input))},
                    {typeof(uint), new DelegatingByteConverter(input => BitConverter.GetBytes((uint) input))},
                    {typeof(ulong), new DelegatingByteConverter(input => BitConverter.GetBytes((ulong) input))},
